...
void FileSystem::update(Entity* entity) {
  if (Input::getKey(KeyCode::S) && terminal_check(TK_CONTROL)) {
    saveFile(entity);
  ...
  } else if (Input::getKey(KeyCode::N) && terminal_check(TK_CONTROL)) {
    saveFile(entity);
    newFile(entity);
  } else if (Input::getKey(KeyCode::R) && terminal_check(TK_CONTROL)) {
    deleteFile(entity);
    selectFile(entity);
  }
}
...
void FileSystem::postUpdate(Entity* entity) {
  ...
  SceneRenderUtility::drawBorder({offsetX, offsetY}, size);
